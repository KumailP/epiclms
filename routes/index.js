...
router.use(function (req, res, next) {
  if (req.isAuthenticated()) { // only collect user info if logged in
    ...
    if(currUser.type == 'student'){
      query = 'SELECT first_name, last_name, photo, email, semester FROM student WHERE student_id = (?)';
    }else{
      query = 'SELECT first_name, last_name, photo, email FROM faculty WHERE faculty_id = (?)';
    }

    db.query(query, [req.user.user_id], (err, results, fields) => {
      if (err) throw err;
